...
def square_list(my_list):
    square_my_list = []
    for elem in my_list:
        square_my_list.append(elem * elem)
        
    return square_my_list
...
def concatenate_lists(list_a, list_b, list_c):
    ...
    for i in range(len(list_a)):
        one_str = list_a[i] + list_b[i] + list_c[i]
        great_list.append(one_str)
...
def rotate_list(input_list, direction, shift):
    start = shift % len(input_list)
    
    if direction == "right":
        start *= -1
        
    shift_list = input_list[start::] + input_list[:start:]
